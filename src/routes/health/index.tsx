...
import { getCurrentUserProfile, getHealthMetricsHistory, logHealthMetric, getTrainerClientsList } from '../../lib/actions'
...
function HealthDashboardPage() {
  ...
  if (!isLoaded || profileLoading && metricsHistory.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh]">
        ...
        <p className="body-md text-[var(--on-surface-variant)] mt-4">Streaming biometric telemetry...</p>
      </div>
    )
  }
  ...
  const getMetricLabel = (type: string) => {
    switch (type) {
      ...
      case 'body_fat': return 'Body Fat Percentage'
  ...
  const getMetricIcon = (type: string) => {
    switch (type) {
      ...
      case 'resting_hr': return 'favorite'
    ...
    if (metricsHistory.length < 2) {
      return (
        <div className="flex flex-col items-center justify-center h-[240px] border border-white/5 bg-white/[0.01] rounded-[var(--rounded-md)] text-center p-6">
          ...
          <h4 className="headline-md font-bold text-white/50 text-sm">Awaiting sufficient timeline data</h4>
          <p className="body-md text-[var(--on-surface-variant)] text-xs mt-1 max-w-xs">
            Submit at least two logs to project premium performance vectors and rolling progress lines.
          </p>
        </div>
      )
    }
        ...
        <div className="flex justify-between items-center">
          <div>
            <h3 className="headline-md text-base font-bold text-white flex items-center gap-2">
              <span className="material-symbols-outlined text-[var(--secondary-container)]">trending_up</span>
              Biometric Performance Vectors
            </h3>
            <p className="body-md text-[var(--on-surface-variant)] text-xs mt-0.5">
              Timeline trend from {chronologicalData[0].date} to {chronologicalData[chronologicalData.length - 1].date}
            </p>
          </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/5 pb-6">
        <div>
          ...
          <p className="body-md text-[var(--on-surface-variant)] m-0">
            Track dated physical variables, monitor chronological progress, and project biological benchmarks.
          </p>
        </div>
        ...
        {isTrainer && (
          <div className="input-group min-w-[200px]">
            <label className="label-md text-xs text-[var(--on-surface-variant)]">Review Athlete Profile</label>
            <select
            ...
            >
              ...
              {clientsList.map(c => (
                <option key={c.client.id} value={c.client.id}>{c.client.name} (Client)</option>
              ))}
            </select>
          </div>
        )}
          ...
          <div className="card space-y-4">
            ...
            {metricsHistory.length === 0 ? (
              <div className="text-center py-12 text-[var(--on-surface-variant)]">
                ...
                <p className="body-md text-xs">No records tracked under {getMetricLabel(activeMetric)} yet.</p>
              </div>
            ) : (
                    ...
                    {metricsHistory.map((m) => (
                      <tr key={m.id} className="border-b border-white/[0.02] hover:bg-white/[0.01]">
                        ...
                        <td className="py-3 text-white font-bold text-sm">
                          {Number(m.value).toFixed(1)} <span className="text-[10px] text-[var(--on-surface-variant)] font-normal">{m.unit}</span>
                        </td>
                        <td className="py-3 text-[var(--on-surface-variant)]">
                          {m.recordedByUserId === m.userId ? 'Individual Core' : 'Trainer Portal Override'}
                        </td>
                        <td className="py-3 text-[var(--on-surface-variant)] truncate max-w-[200px]" title={m.notes || ''}>
                          {m.notes || '--'}
                        </td>
                      </tr>
                    ))}
        ...
        <div className="lg:col-span-1 space-y-6">
            ...
            <form onSubmit={handleMetricSubmit} className="space-y-4">
              ...
              {isTrainer && (
                <div className="input-group">
                  <label className="label-md text-xs text-[var(--on-surface-variant)]">Designate Sync Target</label>
                  <select
              ...
              <div className="input-group">
                <label className="label-md text-xs text-[var(--on-surface-variant)]">Verification / Notes</label>
                <textarea
                  value={logNotes}
          ...
          <div className="card bg-white/[0.01] border-white/5 space-y-3">
            <div className="flex items-center gap-2">
              <span className="material-symbols-outlined text-[var(--primary-container)]">info</span>
              <h4 className="headline-md font-bold text-white text-xs uppercase tracking-wider m-0">Performance Insights</h4>
            </div>
            <p className="body-md text-[var(--on-surface-variant)] text-xs leading-relaxed">
              Consistently recording biometric variables helps map metabolic adaptation charts. Trainers with active partnerships can input telemetry override data, which maps into your charts securely.
            </p>
          </div>
